...
export default function FeaturedProjectsShowcase({
  ...
  const renderProjectCard = (project: FeaturedProject, index: number, mobile: boolean) => (
    <Link
      key={`${project.id}-${index}-${mobile ? "mobile" : "desktop"}`}
    ...
    >
      <Image
        ...
        sizes={mobile ? "84vw" : "(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 25vw"}
      ...
      <div className="absolute left-3.5 top-3.5 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] text-white/75 backdrop-blur max-md:px-2.5 max-md:text-[9px]">
        {index === 0 ? "Premium spotlight" : "Featured project"}
      </div>
      <div className="absolute bottom-0 left-0 right-0 p-4 max-md:p-3.5 text-white">
        <p className="line-clamp-1 text-[11px] uppercase tracking-[0.22em] text-white/58">
          {project.builder}
        </p>
        <h3 className="mt-2 line-clamp-2 text-lg font-semibold leading-tight text-white max-md:mt-1.5 max-md:text-base">
          {project.name}
        </h3>
        <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-white/78 max-md:mt-2">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="hidden gap-3 md:grid md:grid-cols-2 xl:grid-cols-4">
        {visibleProjects.map((project, index) => renderProjectCard(project, index, false))}
      </div>
